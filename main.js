		...
		if (event.data != undefined){
			...
			switch(event.data.topic.slice(0, -1 * (UserID.length+1))){
				case 'following':
					...
					handleEvent(message.user_id, 'Has Followed The Channel')
					break;
				case 'stream-chat-room-v1':
					if (message.data.target_channel_id == UserID){
						console.log("Host" + message.data.channel_login)
						handleEvent(message.data.channel_id, `Hosted With ${message.data.num_viewers} Viewers`)
					}
					break;
				case 'user-bits-events-v1': //Untested
					handleEvent(message.data.channel_id, message.data.chat_message)
					break;
				case 'user-subscribe-events-v1': //Untested
					...
					handleEvent(message.data.channel_id, ret)
					break;
				default:
